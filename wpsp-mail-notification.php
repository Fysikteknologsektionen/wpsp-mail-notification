...
Author: Eric Carlsson (ericca)
GitHub Plugin URI: Fysikteknologsektionen/wpsp-mail-notification
*/
...
function wpsp_mail($ticket_id) {
  ...
  $result = $wpdb->get_row($sql);
	$formatted_create_time = get_date_from_gmt($result->create_time);
  if (!is_null($result)) {
    $mail_to = get_bloginfo('admin_email');
    $mail_subject = 'Ftek Support - Ticket #' . $ticket_id;
    ...
    $mail_message = '<html><font size=4>A new ticket has been created in WP Support Plus</font><br /><b>Author:</b> ' . $result->guest_name
                    ...
                    . '<br /><b>Date:</b> ' . $formatted_create_time
                    . '<br /><a href="' . $support_url . '"><b>View ticket</b></a><br /><br /></html>';
